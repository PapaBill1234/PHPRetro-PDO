...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'savedetails') { Csrf::requireValid();
...
?>
<div class="page_title"><span class="page_name">Users</span></div><div class="page_main"><div class="center"><?php if($notice !== '') { ?><div class="clean-ok"><?php echo $e($notice); ?></div><?php } ?><?php if ($user !== false && $user !== null) { ?><form method="post" action="<?php echo PATH; ?>/housekeeping/users?do=savedetails"><?php echo Csrf::field(); ?><input type="hidden" name="id" value="<?php echo (int) $user['id']; ?>"><p><?php echo $e($user['username']); ?></p><label>Email</label><br><input name="mail" value="<?php echo $e($user['mail']); ?>"><br><label>Rank</label><br><input name="rank" type="number" value="<?php echo (int) $user['rank']; ?>"><br><label>Credits</label><br><input name="credits" type="number" value="<?php echo (int) $user['credits']; ?>"><br><label>Pixels</label><br><input name="pixels" type="number" value="<?php echo (int) $user['pixels']; ?>"><br><label>Points</label><br><input name="points" type="number" value="<?php echo (int) $user['points']; ?>"><br><button>Save</button></form><?php } else { ?><form id="bulk-users" method="post"><?php echo Csrf::field(); ?><input type="hidden" name="bulk_action" value="set_rank"><label>Bulk rank</label><input name="bulk_rank" type="number" value="1"><button>Apply to selected</button><input type="text" name="q" value="<?php echo $e($term); ?>"><button>Search</button></form><table><tr><th>Select</th><th>ID</th><th>User</th><th>Email</th><th>Rank</th><th>Credits</th><th>Pixels</th><th>Points</th><th>Online</th></tr><?php foreach($users as $row){?><tr><td><input form="bulk-users" type="checkbox" name="user_ids[]" value="<?php echo (int)$row['id'];?>"></td><td><?php echo(int)$row['id'];?></td><td><a href="<?php echo PATH;?>/housekeeping/users?do=edit&id=<?php echo(int)$row['id'];?>"><?php echo $e($row['username']);?></a></td><td><?php echo $e($row['mail']);?></td><td><?php echo(int)$row['rank'];?></td><td><?php echo(int)$row['credits'];?></td><td><?php echo(int)$row['pixels'];?></td><td><?php echo(int)$row['points'];?></td><td><?php echo $e($row['online']);?></td></tr><?php }?></table><?php } ?></div></div><?php require_once('./templates/housekeeping_footer.php'); ?>
