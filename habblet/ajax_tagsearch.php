...
$groupCount = habbletGuildTagCount($db, $search);
...
$groups = $groupCount === 0 ? [] : habbletGuildsForTag($db, $search);
?>
...
<p class="search-result-count"><?php echo $count; ?> users. <?php echo $groupCount; ?> groups.</p>
...
<?php } ?></tbody></table>
<?php if ($groups !== []) { ?>
<table class="search-result search-result-groups"><tbody>
<?php foreach ($groups as $group) { ?>
<tr><td><?php if ($group['badge'] !== '') { ?><img src="<?php echo $input->HoloText($group['badge']); ?>" alt="" /><?php } ?></td>
<td><a class="result-title" href="<?php echo PATH.'/groups/'.(int) $group['id'].'/id'; ?>"><?php echo $input->HoloText($group['name']); ?></a><br />
<span class="result-description"><?php echo $input->HoloText($group['description']); ?></span></td></tr>
<?php } ?></tbody></table>
<?php } ?>
