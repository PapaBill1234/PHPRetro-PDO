...
$page['name'] = $lang->loc['pagename.home'] ?? 'Home';
$page['bodyid'] = "landing";
...
case "submit":
	...
	if($user->error > 0){
		$login_error = $lang->loc['error.'.$user->error] ?? 'Login failed.';
		if($user->error == 3){
			$login_error = ($lang->loc['banned.1'] ?? 'Banned: ').($user->banned['reason'] ?? '').($lang->loc['banned.2'] ?? ' expires ').($user->banned['expire'] ?? '').".";
		}
	}
	if(($_SESSION['login']['tries'] ?? 0) > 4 && $settings->find("site_capcha") == "1"){
		if((($_SESSION['register-captcha-bubble'] ?? '') == strtolower((string) ($_POST['bean_captchaResponse'] ?? '')) && !empty($_SESSION['register-captcha-bubble'])) || $settings->find("site_capcha") == "0") {
